...
import { api, type Project, type RequestLog } from '../api.ts';
...
export function ProjectLogs() {
  ...
  const reload = useCallback(async () => {
    const { logs: rows, total: count } = await api.projectLogs(project.slug, {
      limit: 200,
  ...
  }, [project.slug, search, errorsOnly]);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        const { logs: fresh, total: count } = await api.projectLogs(project.slug, {
  ...
  }, [live, project.slug, search, errorsOnly]);
    ...
    <>
      ...
      <div className="log-toolbar">
        <div className="log-search">
          ...
          <input
            placeholder="Search path, host or message…"
            value={search}
          ...
          <EmptyState
            ...
            description={
              search || errorsOnly
                ...
                : `Visit ${project.productionUrl.replace(/^https?:\/\//, '')} and requests will appear here.`
            }
          />
        ) : (
